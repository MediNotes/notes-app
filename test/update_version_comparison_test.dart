...
void main() => group('Update manager:', () {
      ...
      test('Test version comparison (release mode)', () {
        ...
        expect(UpdateManager.getUpdateStatus(v, v + 10),
            UpdateStatus.updateOptional);
        expect(UpdateManager.getUpdateStatus(v, v + 11),
            UpdateStatus.updateOptional);
        expect(UpdateManager.getUpdateStatus(v, v + 19),
            UpdateStatus.updateOptional);

        expect(UpdateManager.getUpdateStatus(v, v + 20),
            UpdateStatus.updateRecommended);
        expect(UpdateManager.getUpdateStatus(v, v + 100),
            UpdateStatus.updateRecommended);
      });
      ...
      test('Test that the latest version can be parsed from version.dart',
          () async {
        // load local file from lib/data/version.dart
        String latestVersionFile =
            await File('lib/data/version.dart').readAsString();
        expect(latestVersionFile.isNotEmpty, true,
            reason: 'Failed to load local version.dart file');

        final int? parsedVersion =
            await UpdateManager.getNewestVersion(latestVersionFile);
        expect(parsedVersion, isNotNull,
            reason: 'Could not parse version number from version.dart file');
        expect(parsedVersion, buildNumber,
