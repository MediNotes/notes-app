...
void main() => group('Test login validation:', () {
      group('Email:', () {
        ...
        test('Invalid email should fail', () {
          expect(
              LoginInputGroup.validateUsername('invalid email @'), isNotNull);
        });
      });
      ...
      group('URL:', () {
        ...
        test('Default URL should pass', () {
          expect(
              LoginInputGroup.validateCustomServer(
                  NextcloudClientExtension.defaultNextcloudUri.toString()),
              isNull);
        });
        test('Invalid URL should fail', () {
          expect(
              LoginInputGroup.validateCustomServer('invalid url'), isNotNull);
        });
        ...
        test('URL with http (not https) should pass', () {
          expect(LoginInputGroup.validateCustomServer('http://example.com'),
              isNull);
        });
        test('URL with ftp protocol should fail', () {
          expect(LoginInputGroup.validateCustomServer('ftp://example.com'),
              isNotNull);
        });
        test('IP address without port should pass', () {
          expect(LoginInputGroup.validateCustomServer('http://192.168.0.1'),
              isNull);
        });
        test('IP address with port should pass', () {
          expect(
              LoginInputGroup.validateCustomServer('http://192.168.0.1:8080'),
              isNull);
        });
      });
